//objectcomments Sélection caractéristique des graines (pour un lot déterminé)
forward
global type w_l_caractg from w_ancestor
end type
type cb_ok from uo_cb_ok within w_l_caractg
end type
type cb_cancel from uo_cb_cancel within w_l_caractg
end type
type dw_1 from uo_ancestor_dwbrowse within w_l_caractg
end type
end forward
...
integer width = 2418
integer height = 2084
string title = "Sélection des caractéristiques des graines"
...
global w_l_caractg w_l_caractg

type variables

end variables

on w_l_caractg.create
int iCurrent
call super::create
...
// récupérer les paramètres (n° lot, date des caractéristiques)
...
IF IsValid(lstr_params) THEN 
	CHOOSE CASE upperbound(lstr_params.a_param)
		...
		CASE 2
			ls_reflot = lstr_params.a_param[1]
			l_dtcaract = gu_datetime.uf_dfromdt(lstr_params.a_param[2])
		CASE ELSE
			gu_message.uf_error("Erreur d'argument")
			post close(this)
			return
...
IF NOT IsNull(l_dtcaract) THEN
	ls_where = ls_where + " and dt_caract='" + string(l_dtcaract, "dd/mm/yyyy") + "'"
END IF
...
IF ll_selrow > 0 THEN
	lstr_params.a_param[1] = dw_1.Object.dt_caract[ll_selrow]
	lstr_params.a_param[2] = dw_1.Object.num_caract[ll_selrow]
