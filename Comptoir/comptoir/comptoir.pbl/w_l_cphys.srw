//objectcomments Sélection d'un test de caractéristiques physiques
forward
global type w_l_cphys from w_ancestor
end type
type cb_ok from uo_cb_ok within w_l_cphys
end type
type cb_cancel from uo_cb_cancel within w_l_cphys
end type
type dw_1 from uo_ancestor_dwbrowse within w_l_cphys
end type
end forward
...
integer width = 2688
integer height = 1916
string title = "Sélection d~'un test de caractéristiques physiques"
...
IF NOT f_isEmptyString(ls_reflot) THEN
	...
	ls_sql = f_modifySQL(ls_sql, "graine_cphys.ref_lot = '" + ls_reflot + "'", "", "")
