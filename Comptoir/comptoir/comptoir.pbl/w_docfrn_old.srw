//objectcomments Encodage des documents fournisseurs - avant 27/11/2013
forward
global type w_docfrn_old from w_ancestor_dataentry
end type
type dw_docfrn from uo_datawindow_singlerow within w_docfrn_old
end type
end forward
...
global w_docfrn_old w_docfrn_old
...
type ddlb_message from w_ancestor_dataentry`ddlb_message within w_docfrn_old
integer y = 1824
integer width = 1719
end type
...
CHOOSE CASE as_item
	CASE "num_df"
		...
		select count(*) into :ll_count from docfrn
				where num_df = :is_docfrn using ESQLCA;
	...
	CASE "dt_caract"
		return(ibr_docfrn.uf_check_dtcaract(as_data, as_message, this.object.ref_lot[al_row]))
		
	CASE "num_caract"
		l_dtcaract = gu_datetime.uf_dfromdt(this.object.dt_caract[al_row])
		return(ibr_docfrn.uf_check_numcaract(as_data, as_message, this.object.ref_lot[al_row], l_dtcaract))
...
CHOOSE CASE idwo_currentItem.name
	...
	CASE "dt_caract"
		...
		openwithparm(w_l_caractg, lstr_params)
		IF Message.DoubleParm = -1 THEN 
			return
		ELSE
			lstr_params=Message.PowerObjectParm
	...
	CASE "num_caract"
		lstr_params.a_param[1] = this.object.ref_lot[al_row]
		lstr_params.a_param[2] = this.object.dt_caract[al_row]
		openwithparm(w_l_caractg, lstr_params)
		IF Message.DoubleParm = -1 THEN 
			return
		ELSE
			lstr_params=Message.PowerObjectParm
			this.uf_setdefaultvalue(al_row, "dt_caract", lstr_params.a_param[1])
			this.SetText(string(lstr_params.a_param[2]))
			f_presskey("tab")
		END IF
...
CHOOSE CASE as_name
	...
	CASE "ref_lot"
		...
		this.object.dt_caract[al_row] = gu_c.date_null
		this.object.num_caract[al_row] = gu_c.i_null
