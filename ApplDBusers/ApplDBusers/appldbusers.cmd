...
REM Les .DLL du runtime PowerBuilder doivent se trouver dans le dossier local spécifié par la 
REM     variable de script PBRTS (normalement C:\pbrts).
REM Les nouvelles versions doivent être copiées soit directement dans le dossier %1, soit
REM     dans le sous-dossier %1%\Upgrade.Dans ce cas, le fichier version.txt indique la mise à jour.
REM
...
REM argument %4 (facultatif) = le dossier où se trouve les exécutables oracle si <> de ceux prévus (voir variable ORABIN).

REM exemple de raccourci : dbcentrale.cmd o:\dbcentrale o:\pbrts
REM			   démarrer en : D:\Dbcentrale
REM			   icone d:\dbcentrale\dbcentrale.exe
...
:syntaxe
echo .
echo . syntaxe de la commande : appldbusers.cmd  dossier_source_programmes  dossier_source_runtimePB  {dossier_local_runtimePB} {dossier_ORACLE_BIN}
echo .
pause

:fin
xcopy /d "%1\appldbusers.cmd" .