//objectcomments Sélection agent
forward
global type w_l_agent from w_ancestor
end type
type sle_nom from uo_sle within w_l_agent
end type
type st_1 from uo_statictext within w_l_agent
end type
type cb_cancel from uo_cb_cancel within w_l_agent
end type
type cb_ok from uo_cb_ok within w_l_agent
end type
type dw_1 from uo_ancestor_dwbrowse within w_l_agent
end type
end forward

global type w_l_agent from w_ancestor
string tag = "TEXT_00595"
integer x = 498
integer width = 3259
integer height = 2016
string title = "Sélection d~'un agent"
...
global w_l_agent w_l_agent

event ue_postopen;call super::ue_postopen;dw_1.retrieve(gu_c.s_null)
...
event we_changed;dw_1.SetFilter("match(upper(nom), ~"" + gu_stringservices.uf_embeddbq(sle_nom.text) + "~")")
dw_1.Filter()
end event

type st_1 from uo_statictext within w_l_agent
string tag = "TEXT_00596"
integer x = 238
integer width = 933
...
IF dw_1.uf_extendedselect() THEN
	DO WHILE ll_selrow > 0
		li_param++
		lstr_params.a_param[li_param] = dw_1.Object.matricule[ll_selrow]
		ll_selrow = dw_1.GetSelectedRow(ll_selrow)
	LOOP
ELSE
	IF dw_1.GetRow() > 0 THEN
		li_param++
		lstr_params.a_param[li_param] = dw_1.Object.matricule[dw_1.GetRow()]
	END IF
END IF
IF li_param = 0 THEN
