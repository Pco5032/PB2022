//objectcomments Liste des agents dont un responsable a accès aux données
forward
global type w_l_encadrement_inv from w_ancestor
end type
type cb_cancel from uo_cb_cancel within w_l_encadrement_inv
end type
type cb_ok from uo_cb_ok within w_l_encadrement_inv
end type
type dw_1 from uo_ancestor_dwbrowse within w_l_encadrement_inv
end type
end forward
...
global w_l_encadrement_inv w_l_encadrement_inv

event ue_postopen;call super::ue_postopen;dw_1.retrieve()

end event

on w_l_encadrement_inv.create
int iCurrent
call super::create
...
on w_l_encadrement_inv.destroy
call super::destroy
...
IF dw_1.uf_extendedselect() THEN
	DO WHILE ll_selrow > 0
		li_param++
		lstr_params.a_param[li_param] = dw_1.Object.prep_matricule[ll_selrow]
		ll_selrow = dw_1.GetSelectedRow(ll_selrow)
	LOOP
ELSE
	IF dw_1.GetRow() > 0 THEN
		li_param++
		lstr_params.a_param[li_param] = dw_1.Object.prep_matricule[dw_1.GetRow()]
	END IF
END IF
IF li_param = 0 THEN
...
integer height = 1456
integer taborder = 10
string dataobject = "d_l_encadrement_inv"
