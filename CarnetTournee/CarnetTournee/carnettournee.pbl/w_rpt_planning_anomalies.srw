//objectcomments Activités planifiées : détection des anomalies d'encodage
forward
global type w_rpt_planning_anomalies from w_ancestor_rptpreview
end type
end forward

global type w_rpt_planning_anomalies from w_ancestor_rptpreview
string tag = "TEXT_00813"
string title = "Vérification de la cohérence des activités planifiées"
end type
global w_rpt_planning_anomalies w_rpt_planning_anomalies
...
on w_rpt_planning_anomalies.create
call super::create
end on

on w_rpt_planning_anomalies.destroy
call super::destroy
end on
...
IF NOT isValid(lstr_params) THEN
	...
	// Argument "P" indique au programme de sélection qu'on traite du planifié.
	OpenWithParm(w_selection_rpt_anomalies_encodage, "P")
...
type cb_defaults from w_ancestor_rptpreview`cb_defaults within w_rpt_planning_anomalies
end type

type st_2 from w_ancestor_rptpreview`st_2 within w_rpt_planning_anomalies
end type

type st_1 from w_ancestor_rptpreview`st_1 within w_rpt_planning_anomalies
end type

type dw_papersize from w_ancestor_rptpreview`dw_papersize within w_rpt_planning_anomalies
end type

type cb_next from w_ancestor_rptpreview`cb_next within w_rpt_planning_anomalies
end type

type cb_prev from w_ancestor_rptpreview`cb_prev within w_rpt_planning_anomalies
end type

type dw_1 from w_ancestor_rptpreview`dw_1 within w_rpt_planning_anomalies
string dataobject = "d_rpt_planning_anomalies"
end type

type em_zoom from w_ancestor_rptpreview`em_zoom within w_rpt_planning_anomalies
end type

type st_zoom from w_ancestor_rptpreview`st_zoom within w_rpt_planning_anomalies
end type

type gb_1 from w_ancestor_rptpreview`gb_1 within w_rpt_planning_anomalies
end type

type dw_paperorient from w_ancestor_rptpreview`dw_paperorient within w_rpt_planning_anomalies
end type
